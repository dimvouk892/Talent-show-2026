@php
    ...
    $showRankingList = (bool) ($talentShow->show_final_overview ?? false);
    $showChart = (bool) ($talentShow->show_final_chart ?? false);
@endphp

<div wire:key="final-overview-{{ $showRankingList ? 'rank' : '' }}-{{ $showChart ? 'chart' : '' }}" class="w-full flex flex-col items-center px-2">
    @if (! empty($showTitle))
    ...
    @if ($showChart)
        @if (! $showRankingList)
            <h1 class="text-2xl sm:text-4xl md:text-5xl font-black text-center mb-6 sm:mb-8">Γράφημα βαθμολογίας</h1>
        @endif
